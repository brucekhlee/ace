...
def _calc_interface(t):
    ...
    '''
    cutoff=8
    sel_AR = atomsel('(protein and (chain A B C) and (within '+str(cutoff)+' of chain R)) or (protein and (chain R) and (within '+str(cutoff)+' of chain A B C))')
    sel_A = atomsel('protein and (chain A B C) and (within '+str(cutoff)+' of chain R)')
    sel_R = atomsel('protein and (chain R) and (within '+str(cutoff)+' of chain A B C)')
    sel_AR.frame = t
    sel_AR.update()
    sel_A.frame=t
    sel_A.update()
...
def get_interface(pdbin, dcdin, Verbose=False):
